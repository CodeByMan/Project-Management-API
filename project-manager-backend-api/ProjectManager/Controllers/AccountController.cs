...
namespace ProjectManager.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<IActionResult> ConfirmEmail([FromBody] ConfirmEmailDto confirmEmailDto)
        {
            ...
            try
            {
                decodedToken = System.Text.Encoding.UTF8.GetString(WebEncoders.Base64UrlDecode(confirmEmailDto.Token));
            }
            catch
            {
        ...
        public async Task<IActionResult> UpdateProfile([FromBody] UpdateProfileDto model)
        {
            ...
            {
                var passwordCheck = await _userManager.CheckPasswordAsync(user, model.CurrentPassword ?? string.Empty);
                if (!passwordCheck)
                {
                    return BadRequest("Current password is incorrect.");
                }
                var passwordChangeResult = await _userManager.ChangePasswordAsync(user, model.CurrentPassword ?? string.Empty, model.NewPassword);
